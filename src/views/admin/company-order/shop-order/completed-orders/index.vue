<template>
  ...
  <b-row class="my-2">
    ...
    <feather-icon
      ...
      icon="EyeIcon"
      size="16"
      @click="$router.push({ name: 'view-company-completed-order-list', params: { id: data.item.id }})"
    />
    </b-button>
    <b-button
                  v-ripple.400="'rgba(255, 255, 255, 0.15)'"
  ...
  <feather-icon
    ...
    icon="FileTextIcon"
    size="16"
    @click="$router.push({ name: 'view-company-completed-invoice', params: { id: data.item.id }})"
  />
  </b-button>
    </div>
    </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getCompanyCompletedOrder(
