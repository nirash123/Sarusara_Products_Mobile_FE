<template>
  <div>
    <b-card no-body class="p-1 bg-primary rounded p-1">
            <b-row>
              <b-col cols="1">
               <feather-icon @click="$router.push('/company-orders?tab=3')" icon="ChevronLeftIcon" class="text-white" size="54"/>
              </b-col>
              <b-col cols="10">
                <h3 class="text-white d-flex justify-content-center">
              <span class="mt-1 text-white">Completed Order (  C_ORD  {{ orders.order.order_id }} ) </span></h3>
              </b-col>
            </b-row>
          </b-card>
  <b-card>
...
export default {
  ...
  methods: {
    ...
    async companyOrderDetails(){
      try {
        const response = await order.companyCompletedOrderDetails(this.$route.params.id)
