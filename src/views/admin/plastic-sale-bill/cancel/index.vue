                ...
                <div class="text-nowrap">
                  <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="outline-primary" class="btn-icon mr-50">
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-plastic-cancel-sale-order', params: { id: data.item.id } })" />
                  </b-button>
                </div>
              </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getCancelledPlasticSalesOrder(
          this.filterQuery, this.currentPage, this.pagination.perPage, this.sortBy, this.sortDesc);
        const dataArray = Response.data.data.map((x) => ({
          id: x.plastic_sale_orders_id,
          order_date: x.plastic_sale_orders_order_date,
          cashier_id: x.plastic_sale_orders_cashier_id,
          lorry_number: x.plastic_sale_orders_lorry_number,
          users_user_name: x.users_user_name,
          suppliers_supplier_name: x.suppliers_supplier_name,
          bill_number: x.plastic_sale_orders_bill_number,
          old_bill_number: x.plastic_sale_orders_old_bill_number,
          order_time: x.plastic_sale_orders_order_time,
          subtotal: 'Rs: ' + (x.plastic_sale_orders_subtotal).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
          discount: 'Rs: ' + (x.plastic_sale_orders_discount).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
          total: 'Rs: ' + (x.plastic_sale_orders_total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
          active_status: x.plastic_sale_orders_active_status
