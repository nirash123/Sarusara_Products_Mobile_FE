...
export default {
  ...
  methods: {
    ...
    handleCardClick(project) {
      ...
      this.$router.push(`/view-plastic-purchas-order/${project.id}`)
      }else{ 
      this.$router.push(`/invoice-plastic-purchas-order/${project.id}`)
      }
    ...
    checkFormValidityLorry() {
      ...
      this.lorry_company_id_status = !!this.lorry.company_id
    ...
    resetModalLorry() {
      ...
      this.lorry.company_id = ''

      this.lorry_lorry_number_status = null
      this.lorry_company_id_status = null
    },
    ...
    async companyLorry() {
      try {
        ...
        const payload = {
          company_id: this.lorry.company_id,
          lorry_number: this.lorry.lorry_number,
        }

        await admin.companyLorryCreate(payload)
    ...
    checkFormValidityPrice() {
      const valid = this.$refs.form.checkValidity()
      this.company_name_status = valid
    ...
    async companySupplier(company_name, address, phone_no) {
      try {
        this.payload = {
        ...
        this.isLoading = true
        const response = await admin.companyCreate(this.payload)
        this.showSuccessMessage('Create Company Successfully')
        this.groupData()
        this.vehicleGroupData()
        this.$refs.table.refresh();
        this.isLoading = false
      } catch (error) {
        error.response.data.data.company_name ? this.showErrorMessage('The Company name has been already taken.') : ''
    ...
    async groupData() {
      try {
        const response = await admin.companyData()
    ...
    async vehicleGroupData() {
      try {
        const response = await admin.companyLorryData()
    ...
    async addSubItemData(value) {
      if (value === 'other' || value === 'other' || value?.id === 'other') {
        ...
        try {
          const response = await driver.getCompanyLorryDetails(value)

          this.form.lorry_number = response.data.lorry_number
          this.form.user_name = response.data.company.company_name
          this.new_company_id = response.data.company.id
          this.company_status = response.data.status
          this.selected_status = response.data.status
          this.form.phone_no = response.data.company.phone_no
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getNewPlasticPurchaseOrders(
    ...
    async addNewOrder() {
      ...
      try {
        const payload = {
          ...
          company_id: this.selected_status != 6
            ? this.company_id
            : this.new_company_id,
          driver_id: this.driver_id,
        }

        await order.orderPlasticPurchaseCreate(payload)
