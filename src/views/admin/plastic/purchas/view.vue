  ...
  <div>
    <section class="invoice-preview-wrapper">
      <b-row class="invoice-actions invoice-header">
        ...
        <b-col cols="12">
          <b-card class="plastic-header-card-pv">
            ...
            <div class="d-none d-md-block">
              ...
              <div class="cart-container scroll-cart">
                ...
                <div v-for="product in sale_carts" :key="product.id" class="cart-row d-flex align-items-center">
                  ...
                  <div class="col-price">
                    <div class="price-box">
                      <input v-model="product.company_price" :ref="'priceNameInput_' + product.id"
                        @focus="isPriceActive = true" @blur="isPriceActive = false" @select="isPriceActive = true"
                        @mouseup="isPriceActive = true"
                        @change="updateSellSalePriceCart(product.id, product.company_price)" />
                    </div>
                  </div>
          ...
          <b-card class="summary-card shadow-lg rounded-lg sticky-top py-0 px-2">
            ...
            <div>
              ...
              <div class="d-flex flex-column gap-1 ">
                ...
                <div class="info-row">
                  <span v-if="company_data.loan > 0" class="text-primary label">Outstanding Loan <span
                      class="badge badge-primary">
                      {{ formatPrice(company_data.loan) }}
                    </span></span>
                  <span v-if="company_data.buy > 0" class="text-danger label">Credit Amount <span
                ...
                <div v-if="company_data.tbs > 0" class="info-row">
            ...
            <!-- <div v-if="company_data.id == 1" class="input-row">
            ...
            <div class="input-row">
              <div class="d-flex align-items-center">
                ...
                <span class="pcredit">Credit</span>
              </div>
              <b-form-input :disabled='cash >= sum_total' v-model="credit" type="number" min="0" step="0.01"
            ...
            <div class="d-flex justify-content-between mb-3">
              ...
              <h3 v-else class="font-weight-bold text-warning">Rs {{ formatPrice(balance) }}</h3>
            </div>


            <!-- Button -->
            <b-button block class="payment-btn"
              :disabled="balance < 0 || sub_sum_total == 0 || isLoading || isQtyActive || isPriceActive"
              @click="submitNewData">
                ...
                <div v-for="(product, index) in sale_item_carts" :key="product.id" class="item-row">
                  <b-row class="align-items-center no-gutters">
                    ...
                    <b-col cols="2" class="text-right">
                      <h6 class="display-new-71 mb-0">
                        {{ formatPrice(product.company_price) }}
                      </h6>
                    </b-col>
              ...
              <b-row>
                ...
                <b-col cols="6" class="pl-5">
                  <div class="payment-box">
                    ...
                    <div v-if="credit === 0" class="payment-row display-new-70">
                      <span> Change Rs.</span>
  ...
  methods: {
    ...
    async submitNewData() {
      try {
        ...
        this.isLoading = true
        await item.submitPlasticPurchaseCart(this.payload)
        this.printInvoice()
        this.transport = 0
        this.labour = 0
        this.cash = 0
        ...
        this.cheque = 0
        this.credit = 0
        this.$router.push('/plastic-purchase')
    ...
    async updateSaleQuentityCart(id, quentity) {
      ...
      try {
        ...
        await item.updatePlasticPurchaseQuentityCart(id, payload)
    ...
    async updateSellSalePriceCart(id, price) {
      ...
      try {
        ...
        await item.updatePlasticPurchasePriceCart(id, payload)
    ...
    async deleteSaleCart(id) {
      ...
      await item.deletePlasticPurchaseCart(id)
      await this.getAdminSaleCarts()
      if (this.$refs.nameInput) {
        if (this.$refs.nameInput) {
          this.$refs.nameInput.focus()
    ...
    async deleteSaleCartLastItem() {
      ...
      await item.deletePlasticPurchaseCartLastItem(this.$route.params.id)
      await this.getAdminSaleCarts()
      if (this.$refs.nameInput) {
        if (this.$refs.nameInput) {
          this.$refs.nameInput.focus()
    ...
    async addtocart(project) {
      ...
      try {
        ...
        this.isLoading = true
        const response = await item.addProductToPlasticPurchaseCart(this.payload)
    ...
    async deleteOrder() {
      ...
      try {
        ...
        this.$swal({
          ...
          if (result.value) {
            await item.deletePlasticPurchaseOrder(this.order_data.id)
    ...
    async getAdminSaleCarts() {
      try {
        const response = await item.getAdminPlasticPosPurchaseCarts(this.$route.params.id)
    ...
    async getOrderDetails() {
      try {
        const response = await order.getPlasticPurchaseOrderDetails(this.$route.params.id)
    ...
    async getTotalPrice() {
      ...
      if (this.hold < 0 || this.hold === null || this.hold === undefined || this.hold === '' || isNaN(this.hold)) {
        this.hold = 0
      }

      if (this.hold > this.sale_total) {
        this.hold = 0
      }

      if (this.cash >= this.sale_total) {
        this.credit = 0
      }

      if ((this.sale_total - this.cash) < this.credit) {
        this.credit = 0
      }
...
.plastic-header-card-pv {
  background: linear-gradient(135deg, #b38023, #ce9519);
  border-radius: 12px;
  color: white;
  padding: 10px 15px;
}
...
/* Back button */
.plastic-back-btn-pv {
  background: linear-gradient(135deg, #b38023, #b38023);
  border: none;
  color: white;
  border-radius: 10px;
}
