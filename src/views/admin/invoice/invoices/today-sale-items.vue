                ...
                <div v-for="product in dates" :key="product.id" class="px-2 mt-1">
                  ...
                  <div>
                    ...
                    <h3 class="newonetext mb-25 d-flex justify-content-center">
                      <div>Today Sale Items Details Report</div>
                    </h3>
                    <h3 class="newonetext mb-25 d-flex justify-content-center">
                      <div>අද දින විකුණුම් අයිතම විස්තර</div>
                    </h3>
                          ...
                          <b-row v-if="income.date == product.date" class="px-2">
                            ...
                            <b-col cols="2" class="">
                              <h4  class="d-flex justify-content-center"> {{
                                income.sale_price }} </h4>
                            </b-col>
                            <b-col cols="2">
                              <h4  class="d-flex justify-content-center"> {{ income.quentity }} Kg </h4>
                            </b-col>
                            <b-col cols="4">
                              <div class="d-flex justify-content-end">
                                <h4>   Rs: {{ (income.income_total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
                                </h4>
                              </div>
                            </b-col>
                          </b-row>
                  ...
                  <b-col cols="12" class="pt-1 border-primary">
                    <div v-for="income in incomes" :key="income.id">
                      <b-row v-if="income.date == product.date" class="px-2">
                        ...
                        <b-col cols="4">
                          <div class="d-flex justify-content-end">
                            <h4>   Rs: {{ (income.income_total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }} </h4>
                          </div>
                        </b-col>
                      </b-row>
                      <hr class="newonegreen" />
  ...
  methods: {
    ...
    async categoryAdd() {
      try {
        ...
        const status = await invoice.getTodaySaleItemsDetailInvoices(this.payload);
        ...
        async add() {
      try {
        const status = await invoice.getTodaySaleAllItemsDetailInvoices();
        this.dates = status.data.date
        this.incomes = status.data.income
        this.totalIncome = status.data.total_income
        this.totalOldIncome = status.data.total_old_income
