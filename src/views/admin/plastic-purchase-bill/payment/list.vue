<template>
  <div class="fixed-page">
    <section class="invoice-preview-wrapper">
        ...
        <b-row>
          <b-col cols="1">
            <feather-icon @click="$router.push('/plastic-order_list?tab=3')" icon="ChevronLeftIcon" class="text-white"
              size="54" />
          </b-col>
          <b-col cols="10">
            <h3 class="text-white d-flex justify-content-center">
              <span class="mt-1 text-white">Payment Plastic Purchase Order ( {{ orders.plastic_purchase_orders_bill_number }} ) </span>
            </h3>
          </b-col>
        </b-row>
      </b-card>
      <b-row class="invoice-actions">
        <b-col lg="6" md="6" sm="12" class="mb-2">
          <div class="ml-2">
            ...
            <b-card-text>
              {{ orders.plastic_purchase_orders_bill_number }}
            </b-card-text>
          </div>
        ...
        <b-col lg="6" md="6" sm="12" class="mb-2">
          ...
          <div class="ml-2">
            ...
            <b-card-text>
              <div>
                Rs: {{ (orders.plastic_purchase_orders_total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
              </div>
            </b-card-text>
          </div>
          <div class="ml-2">
            ...
            <b-card-text>
              {{ orders.plastic_purchase_orders_order_date }} | {{ orders.plastic_purchase_orders_order_time }}
            </b-card-text>
          </div>
          <div class="ml-2 mb-1">
            <h4 class="text-capitalize mt-2">
          ...
          <div v-if="orders.plastic_purchase_orders_cash > 0" class="ml-2">
            <h4 class="text-capitalize mt-2">
              Cash
            </h4>
            <b-card-text>
              Rs: {{ (orders.plastic_purchase_orders_cash).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
            </b-card-text>
          </div>
            <div v-if="orders.plastic_purchase_orders_hold > 0" class="ml-2">
            <h4 class="text-capitalize mt-2">
              Card
            </h4>
            <b-card-text>
              Rs: {{ (orders.plastic_purchase_orders_hold).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
            </b-card-text>
          </div>
          <div v-if="orders.plastic_purchase_orders_cheque > 0" class="ml-2">
            <h4 class="text-capitalize mt-2">
              Cheque
            </h4>
            <b-card-text>
              <div>
                Rs: {{ (orders.plastic_purchase_orders_cheque).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
              </div>
            </b-card-text>
          </div>
          <div v-if="orders.plastic_purchase_orders_credit > 0" class="ml-2">
            <h4 class="text-capitalize mt-2">
              Credit
              ...
              <b-modal id="modal-prevent-closing-price" centered ref="my-modal-price"
                :title="`Add New Payment  ( Rs:  ${orders.plastic_purchase_orders_credit} )`" ok-title="Submit"
            ...
            <b-card-text>
              <div>
                Rs: {{ (orders.plastic_purchase_orders_credit).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
              </div>
            </b-card-text>
          </div>
          <!-- <span class="ml-1">
              <b-button v-if="orders.plastic_purchase_orders_active_status == 0" :class="{ 'btn-loading': isLoading }"
                :disabled="isLoading" type="submit" variant="primary" @click="addStore(orders.plastic_purchase_orders_id)">
          ...
          <span class="ml-1">
            <!-- <b-button v-if="orders.plastic_purchase_orders_active_status == 0" :class="{ 'btn-loading': isLoading }"
                :disabled="isLoading" type="submit" variant="danger" @click="deleteOrder(orders.plastic_purchase_orders_id)">
                <span v-if="isLoading">Loading...</span>
      ...
      <b-card style="height: 805px;" class="invoice-actions">
        ...
        <div>
          <h4 class="d-flex align-items-center justify-content-start mb-1">
            Order Items List : ( Rs: {{ (orders.plastic_purchase_orders_total).toString(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
            )
          </h4>
        </div>
        <table style="height: 605px;" class="table table-hover shopping-cart-wrap table-responsive">
          ...
          <tbody>
            <tr v-for="product in item_orders" :key="product.plastic_purchase_orders_id">
              <td>
                <figure class="media">
                ...
                <figure class="media">
                  <figcaption class="media-body mt-1">
                    <h6 class="title text-truncate">Rs: {{
                      product.sale_orders_company_price
                        ? Number(product.sale_orders_company_price).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g,
                          ",")
                        : '0'
                      }} </h6>
                  </figcaption>
                  ...
                  <b-row>
                    ...
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8">
                        Invoice No <span class="ml-3">: {{ orders.plastic_purchase_orders_bill_number }} </span>
                      </h6>
                      <h6 class="font-weight-bolder display-new-8">
                    ...
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8 d-flex justify-content-end">
                        Invoice Date <span class="ml-3"> : {{ formatDate(orders.plastic_purchase_orders_order_date) }} </span>
                      </h6>
                      <h6 class="font-weight-bolder display-new-8 d-flex justify-content-end">
                        {{ formatTime(orders.plastic_purchase_orders_order_time) }}
                      </h6>
                    </b-col>
                       <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8">
                        Company <span class="ml-3"> <span class="ml-50">: {{ orders.companies_company_name.substring(0, 22) }}
                          </span></span>
                      </h6>
                    </b-col>
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8 d-flex justify-content-end">
                       Lorry Number <span class="ml-3"> <span class="ml-50">: {{ orders.plastic_purchase_orders_lorry_number }}
                          </span></span>
                      </h6>
                    </b-col>
                  ...
                  <div v-for="(product, index) in item_orders" :key="product.id" class="item-row">
                    <b-row class="align-items-center no-gutters">
                      ...
                      <b-col cols="2" class="text-right">
                        <h6 class="display-new-71 mb-0">
                          {{ formatPrice(product.sale_orders_company_price) }}
                        </h6>
                      </b-col>
                  ...
                  <b-col cols="6" class="pl-5">
                    <div class="payment-box">
                      <div class="payment-row display-new-73">
                        ...
                        <span> {{ formatPrice(orders.plastic_purchase_orders_total) }}</span>
                      </div>
                      <div class="payment-row display-new-70">
                        <span>Amount Paid </span>
                        <span> {{ formatPrice(orders.plastic_purchase_orders_cash) }}</span>
                      </div>

                      <div class="payment-row display-new-70">
                        <span> Credit Rs. </span>
                        <span v-if="orders.plastic_purchase_orders_credit > 0"> {{ formatPrice(orders.plastic_purchase_orders_credit)
                          }}</span>
                        <span v-else> 0.00</span>
                      </div>

                      <div v-if="orders.plastic_purchase_orders_credit === 0" class="payment-row display-new-70">
                        <span> Change Rs.  </span>
                        <span v-if="orders.plastic_purchase_orders_labour_charge > 0"> {{
                          formatPrice(orders.plastic_purchase_orders_labour_charge) }}</span>
                        <span v-else> 0.00</span>
                      </div>
  ...
  methods: {
    ...
    async addStore(id) {
     ...
     async saleOrderDetails() {
      try {
        const response = await order.pastPlasticPurchasOrderDetails(this.$route.params.id)
        this.orders = response.data.data

        this.saleOrderList(this.orders.plastic_purchase_orders_id)
      } catch (error) {
      }
    },
    ...
    async saleOrderList(id) {
      try {
        const response = await order.plasticPurchasOrderList(id)
    ...
    async companyPrice(new_cash = 0, new_card = 0, new_cheque = 0) {
      ...
      const formattedPrice = parseFloat(this.orders.plastic_purchase_orders_credit) || 0;
      ...
      try {
        ...
        await item.submitPlasticPurchaseNewCredit(this.$route.params.id, payload);
