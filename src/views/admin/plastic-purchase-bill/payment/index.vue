                ...
                <div class="text-nowrap">
                  <b-button v-if="data.item.active_status != 3" v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                    ...
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-payment-plastic-purchase-order', params: { id: data.item.id } })" />
                  </b-button>
                </div>
              </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getPaymentPlasticPurchasOrder(
        ...
        const dataArray = Response.data.data.map((x) => ({
          ...
          order_time: new Date('1970-01-01T' + x.plastic_purchase_orders_order_time)
            .toLocaleTimeString('en-US', {
              hour: '2-digit',
              minute: '2-digit',
              hour12: true
            }),
          subtotal: Number(x.plastic_purchase_orders_subtotal).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          credit: 'Rs: ' + Number(x.plastic_purchase_orders_credit).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          total: 'Rs: ' + Number(x.plastic_purchase_orders_total).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),
