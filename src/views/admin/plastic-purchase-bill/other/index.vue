                ...
                <div class="text-nowrap">
                  <b-button v-if="data.item.active_status != 3" v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                    ...
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-past-plastic-purchase-order', params: { id: data.item.id } })" />
                  </b-button>
                </div>
              </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getPastPlasticPurchasOrder(
        ...
        const dataArray = Response.data.data.map((x) => ({
          ...
          hold: 'Rs: ' + Number(x.plastic_purchase_orders_hold).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),
