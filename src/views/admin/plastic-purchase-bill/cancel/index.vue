                ...
                <div class="text-nowrap">
                  <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="outline-primary" class="btn-icon mr-50">
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-cancel-purchase-order', params: { id: data.item.id } })" />
                  </b-button>
                </div>
              </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getCancelledPlasticPurchasOrder(
          this.filterQuery, this.currentPage, this.pagination.perPage, this.sortBy, this.sortDesc);
        const dataArray = Response.data.data.map((x) => ({
          id: x.plastic_purchase_orders_id,
          order_date: x.plastic_purchase_orders_order_date,
          cashier_id: x.plastic_purchase_orders_cashier_id,
          users_user_name: x.users_user_name,
          companies_company_name: x.companies_company_name,
          bill_number: x.plastic_purchase_orders_bill_number,
          lorry_number: x.plastic_purchase_orders_lorry_number,
          order_time: x.plastic_purchase_orders_order_time,
          subtotal: 'Rs: ' + (x.plastic_purchase_orders_subtotal).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
          discount: 'Rs: ' + (x.plastic_purchase_orders_discount).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
          total: 'Rs: ' + (x.plastic_purchase_orders_total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
          active_status: x.plastic_purchase_orders_active_status
