                ...
                <div class="text-nowrap">
                  <b-button v-if="data.item.active_status != 3" v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                    ...
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-payment-sell-sale-order', params: { id: data.item.id } })" />
                  </b-button>
                </div>
              </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getPaymentPurchasOrder(
          this.filterQuery, this.currentPage, this.pagination.perPage, this.sortBy, this.sortDesc);
        const dataArray = Response.data.data.map((x) => ({
          id: x.sell_sale_orders_id,
          order_date: x.sell_sale_orders_order_date,
          cashier_id: x.sell_sale_orders_cashier_id,
          users_user_name: x.users_user_name,
          companies_company_name: x.companies_company_name,
          bill_number: x.sell_sale_orders_bill_number,
          order_time: new Date('1970-01-01T' + x.sell_sale_orders_order_time)
            .toLocaleTimeString('en-US', {
              hour: '2-digit',
              minute: '2-digit',
              hour12: true
            }),
          subtotal: Number(x.sell_sale_orders_subtotal).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          credit: 'Rs: ' + Number(x.sell_sale_orders_credit).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          total: 'Rs: ' + Number(x.sell_sale_orders_total).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),
          active_status: x.sell_sale_orders_active_status
        }));
