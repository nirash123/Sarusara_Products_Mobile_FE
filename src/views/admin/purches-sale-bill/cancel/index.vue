                ...
                <div class="text-nowrap">
                  <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                    ...
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-cancel-sell-sale-order', params: { id: data.item.id } })" />
                  </b-button>
                </div>
              </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getCancelledPurchasOrder(
        ...
        const dataArray = Response.data.data.map((x) => ({
          ...
          hold: 'Rs: ' + Number(x.sell_sale_orders_hold).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),
