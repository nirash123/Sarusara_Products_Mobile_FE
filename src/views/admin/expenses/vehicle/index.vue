  ...
  <template #row-class="item">
    ...
    {{ item.expense_name === 'TOTAL'
      ...
      : ''
              ...
              <div v-if="vehicles_expenses.length == 0" class="empty-state">
                No Vehicle Expenses
              </div>
  ...
  methods: {
    ...
    async loadExpenses() {
      ...
      try {
        ...
        const response = await admin.getVehicleExpenses(payload)
        ...
        */
        ...
        this.vehicles_expenses = response.data.vehicle_expenses.map(x => ({
