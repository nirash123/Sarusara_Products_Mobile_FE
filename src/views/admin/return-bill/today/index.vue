<template>
    <div>
        <b-card>
        <h2>  Today Return Orders :  {{ pagination.totalRows }} </h2>
    ...
    <b-row class="my-2">
      ...
      <div class="text-nowrap">
        <b-button  v-if="data.item.active_status != 3"
      ...
      <feather-icon
        ...
        icon="EyeIcon"
        size="16"
        @click="$router.push({ name: 'view-payment-today-sale-return-order', params: { id: data.item.id }})"
      />
      </b-button>
      <b-button
      v-if="data.item.active_status == 1 || data.item.active_status == 2"
                    v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                    variant="outline-primary"
                    class="btn-icon mr-50"
                  >
    ...
    async deleteOrder(id) {
      ...
      try {
        ...
        this.$swal({
          ...
          if (result.value) {
          await item.deleteSaleReturnOrder(id)
