<template>
    ...
    <b-card class="p-0">
      ...
      <div class="my-1">
        <div class="d-flex justify-content-center">
          <h2><b>{{ selectedDate }} - PAYMENTS</b></h2>
        </div>
        <b-row>
          <!-- Per Page -->
 ...
 <template #cell(attendance)="data">
    ...
    <b-card class="p-0">
      <div class="d-flex justify-content-center">
        <h2><b>{{ selectedDate }} - PAYMENT PENDING</b></h2>
      </div>
      <div class="mt-2">
        <b-row>
          <!-- Per Page -->
      ...
      <b-row class="m-2">
        <b-col cols="12">
          <b-overlay :show="tableLoading" rounded="sm">
            <b-table ref="table" :current-page="currentPage2" :fields="fields2" :items="payment_pending_carts"
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await attendence.getPastPayments(
        ...
        return this.payment_carts = dataArray
      } catch (error) {
    ...
    async getOldUsers() {
      try {
        ...
        const Response = await attendence.getPastPendingPayments(
          this.selectedDate, this.filterQuery, this.currentPage, this.pagination.perPage, this.sortBy, this.sortDesc);
        const dataArray = Response.data.data.map((x) => ({
          id: x.id,
        ...
        const paginationResponse = Response.data;
        this.currentPage = paginationResponse.current_page;
        this.pagination2.totalRows = paginationResponse.to;
        ...
        this.tableLoading = false;
        return this.payment_pending_carts = dataArray
      } catch (error) {
        this.convertAndNotifyError(error);
        this.tableLoading = false;
      }
