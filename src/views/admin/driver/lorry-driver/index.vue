                ...
                <b-row>
                    ...
                    <b-col cols="12" md="7">
                        <div class="d-flex align-items-center justify-content-end">
                            <b-form-input v-model="filters['name_or_code']" class="d-inline-block modern-search mr-1"
                            ...
                            <b-modal size="lg" id="modal-prevent-closing-price" centered ref="my-modal-price"
                                title="Add New" ok-title="Submit" cancel-variant="outline-secondary"
                                @show="resetModalPrice" @hidden="resetModalPrice" @ok="handleOkPrice">
                                <form ref="form" @submit.stop.prevent="handleSubmitPrice">

                                    <!-- Lorry -->
                                    <b-form-group label="Lorry">
                                        <v-select v-model="lorry_id" label="label" :options="groups"
                                            :reduce="item => item.id" class="custom-v-select" required />
                                    </b-form-group>
                                    <!-- Driver -->
                                    <b-form-group label="Driver" class="mb-5">
                                        <v-select v-model="driver_id" label="label" :options="drivers"
                                            :reduce="item => item.id" class="custom-v-select" required />
                                    </b-form-group>
                                </form>
                            </b-modal>
                ...
                <b-col cols="12">
                    <b-overlay :show="tableLoading" rounded="sm">
                        <b-table ref="table" :current-page="currentPage" :fields="fields" :items="getUsers"
                            ...
                            <template #cell(lorry_code)="data">
                                {{ data.item.lorry_code }}
                            </template>
                            ...
                            <template #cell(actions)="data">
                                <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="outline-danger"
                                    ...
                                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="TrashIcon"
                                        size="16" @click="deleteLorryDriver(data.item.id)" />
                                </b-button>
                            </template>
...
export default {
    ...
    data() {
        return {
            ...
            companyData: {
                first_name: 'Sarusara',
                last_name: 'Products',
                address: 'No 123 Nagaswewa Dehiaththakandiya',
                phone_no: '94717305355'
            },
    ...
    methods: {
        async groupData() {
            try {
                const response = await driver.lorryData()
        ...
        handleSubmitPrice() {
            ...
            this.lorryDriverSubmit()
        },

        async lorryDriverSubmit() {
            try {
                this.isLoading = true

                const payload = {
                    lorry_id: this.lorry_id,
                    driver_id: this.driver_id,
                }

                await driver.lorryDriverCreate(payload)

                this.showSuccessMessage('Create Lorry Driver Successfully')
                this.$refs.table.refresh()
                ...
                if (error.response?.data?.data?.code) {
                    this.showErrorMessage('The Lorry Driver has been already taken.')
                }
            } finally {
