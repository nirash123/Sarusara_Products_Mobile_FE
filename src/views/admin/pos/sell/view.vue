  ...
  <div>
    <section class="invoice-preview-wrapper">
      <b-row class="invoice-actions invoice-header">
        ...
        <b-col cols="12">
          <b-card class="header-card-sell-sell">
            ...
            <div class="d-none d-md-block">
              ...
              <div class="cart-container scroll-cart">
                ...
                <div v-for="product in sale_carts" :key="product.id" class="cart-row d-flex align-items-center">
                  ...
                  <div class="col-price">
                    <div class="price-box">
                      <input v-model="product.sale_price" :ref="'priceNameInput_' + product.item_id"
                        @focus="isPriceActive = true" @blur="isPriceActive = false" @select="isPriceActive = true"
                      @mouseup="isPriceActive = true"
                        @change="updateSellSalePriceCart(product.id, product.sale_price)" />
                    </div>
                  </div>
                  ...
                  <div class="col-qty">
                    <div class="qty-box">
                      ...
                      <input v-model="product.quentity" :ref="'quantityNameInput_' + product.item_id"
                        @focus="isQtyActive = true" @blur="isQtyActive = false" @select="isQtyActive = true"
                      @mouseup="isQtyActive = true"
                        @change="updateSellSaleQuentityCart(product.id, product.quentity)" />
          ...
          <b-card class="summary-card shadow-lg rounded-lg sticky-top py-0 px-2">
            ...
            <div class="d-flex justify-content-between align-items-start">
              ...
              <h4 class="order-title d-flex align-items-center font-weight-bold mb-2">
                ...
                Order Summary ( {{ company_data.company_name }} )
              </h4>
            ...
            <div>
              ...
              <div class="d-flex flex-column gap-1">
                ...
                <div class="info-row">
                  <h4><span v-if="company_data.loan > 0" class="text-primary">Outstanding Loan <span
                      class="badge badge-primary">
                      {{ formatPrice(company_data.loan) }}
                    </span></span><span v-else></span></h4>
                 <h4> <span v-if="company_data.buy > 0" class="text-danger">Credit Amount <span
                      class="badge badge-danger">
                      {{ formatPrice(company_data.buy) }}
                    </span></span></h4>
                </div>

              </div>
              <div v-if="company_data.tbs > 0">
                  <h4><span class="text-success label">TBS Credit Amount <span
                        class="badge badge-success">
                        {{ formatPrice(company_data.tbs) }}
                      </span></span> </h4>
                </div>
            </div>
            ...
            <div class="input-row">
              ...
              <b-form-input ref="cardInput" v-model="hold" type="number" min="0" step="0.01" class="input-box"
                @change="getTotalSalePrice" />
            </div>
            ...
            <div class="input-row">
              <div class="d-flex align-items-center">
                ...
                <span class="pscredit">Credit</span>
              </div>
            ...
            <!-- Button -->
            <b-button block class="payment-btn" :disabled="cash < 0 || sub_sum_total == 0 || isLoading || isQtyActive || isPriceActive"
              @click="submitNewData">
                ...
                <b-row>
                  <b-col cols="12" class="d-flex justify-content-between align-items-center">
                    ...
                    <h6 class="font-weight-bolder display-new-8 mb-0">
                      Company
                      <span class="ml-3"> <span class="ml-50">: {{ order_data.company_name.substring(0, 22) }}
                        </span></span>
                    </h6>
  ...
  methods: {
    ...
    handleKeydown(event) {
      ...
      if (event.key.toLowerCase() === 'enter') {
        if (!this.enterPressCount) {
          ...
          if (this.cash >= 0 && this.sub_sum_total > 0 && !this.isLoading && !this.isQtyActive && !this.isPriceActive) {
            this.submitNewData();
          }
    ...
    async submitNewData() {
      try {
        ...
        this.isLoading = true
        await item.submitSellSaleCart(this.payload)
        this.printInvoice()
        this.transport = 0
        this.labour = 0
        this.cash = 0
    ...
    increaseQty(product) {
      product.quentity++
      this.updateSellSaleQuentityCart(product.id, product.quentity)
    },
    ...
    async updateSellSaleQuentityCart(id, quentity) {
      this.isLoading = true
      ...
      try {
        ...
        await item.updateSellSaleQuentityCart(id, payload)
    ...
    async updateSellSalePriceCart(id, price) {
      this.isLoading = true
      ...
      try {
        ...
        await item.updateSellSalePriceCart(id, payload)
    ...
    async deleteSaleCart(id) {
      ...
      await item.deleteSellSaleCart(id)
      await this.getAdminSaleCarts()
      if (this.$refs.nameInput) {
        if (this.$refs.nameInput) {
          this.$refs.nameInput.focus()
    ...
    async deleteSaleCartLastItem() {
      ...
      await item.deleteSellSaleCartLastItem(this.$route.params.id)
      await this.getAdminSaleCarts()
      if (this.$refs.nameInput) {
        if (this.$refs.nameInput) {
          this.$refs.nameInput.focus()
    ...
    async addtocart(project) {
      ...
      try {
        ...
        this.isLoading = true
        const response = await item.addProductToSaleTodayCart(this.payload)
        ...
        this.$nextTick(() => {
          const input = this.$refs['priceNameInput_' + ItemId]
    ...
    async deleteOrder() {
      ...
      try {
        ...
        this.$swal({
          ...
          if (result.value) {
            await item.deleteSellSaleOrder(this.order_data.id)
    ...
    async getAdminSaleCarts() {
      try {
        const response = await item.getAdminPOSSellSaleCarts(this.$route.params.id)
    ...
    async getOrderDetails() {
      try {
        const response = await order.getSellOrderDetails(this.$route.params.id)
        this.order_data = response.data.order
        this.company_data = response.data.company
      } catch (error) {
...
.header-card-sell-sell {
  background: linear-gradient(135deg, #681e8a, #ac25eb);
  border-radius: 12px;
  color: white;
  padding: 10px 15px;
}
...
/* Back button */
.back-btn-sell {
  background: linear-gradient(135deg, #681e8a, #681e8a);
  border: none;
  color: white;
  border-radius: 10px;
}
