            ...
            <div class="d-none d-md-block">
              ...
              <div class="cart-container scroll-cart">
                ...
                <div v-for="product in sale_carts" :key="product.id" class="cart-row d-flex align-items-center">
                  ...
                  <div class="col-price">
                    Rs. {{ formatPrice(product.purchas_price) }}
                  </div>
                  ...
                  <div class="col-qty">
                    <div class="qty-box">
                      ...
                      <input v-model="product.quentity" :ref="'quantityNameInput_' + product.id"
                        ...
                        @mouseup="isQtyActive = true" @change="updateSaleQuentityCart(product.id, product.quentity)" />
                  ...
                  <div class="col-total text-success">
                    Rs. {{ formatPrice(product.quentity * product.purchas_price) }}
                  </div>
            ...
            <div class="mt-5 d-block d-md-none">
              ...
              <div v-for="product in sale_carts" :key="product.id" class="mobile-card">
                ...
                <div class="d-flex justify-content-between mb-2">
                  ...
                  <span>Rs. {{ formatPrice(product.purchas_price) }}</span>
                </div>
        ...
        <b-col cols="12" lg="5">
          ...
          <b-card class="summary-card shadow-lg rounded-lg sticky-top py-0 px-2">
            ...
            <div class="d-flex justify-content-between align-items-start">
              ...
              <h4 class="order-title d-flex align-items-center font-weight-bold mb-2" id="toggle-btn-price"
                v-b-modal.modal-prevent-closing-price>
                <span class="icon-box blue mr-2">🛒</span>
                Order Summary ( {{ supplier_data.supplier_name }} <span v-if="driver_data"> - {{ driver_data.first_name
                }} {{ driver_data.last_name }} </span>)
              </h4>
            ...
            <div>
              ...
              <div class="d-flex flex-column gap-1">
                <div v-if="driver_data" class="info-row">
                  <h4 @click="$router.push('/admin-loan')"><span v-if="driver_data.loan > 0" class="text-primary label">Outstanding Loan <span
                        class="badge badge-primary">
                        {{ formatPrice(driver_data.loan) }}
                      </span></span><span v-else></span></h4>
                  <h4 @click="$router.push('/admin-credit?tab=3')"><span v-if="driver_data.buy > 0" class="text-danger label">Credit Amount <span
                        class="badge badge-danger">
                        {{ formatPrice(driver_data.buy) }}
                      </span></span></h4>
                </div>
                <div v-else class="info-row">
                  <h4 @click="$router.push('/admin-loan')"><span v-if="supplier_data.loan > 0" class="text-primary label">Outstanding Loan <span
                        class="badge badge-primary">
                        {{ formatPrice(supplier_data.loan) }}
                      </span></span> <span v-else></span></h4>
                  <h4  @click="$router.push('/admin-credit?tab=3')"><span v-if="supplier_data.buy > 0" class="text-danger label">Credit Amount <span
                        class="badge badge-danger">
                        {{ formatPrice(supplier_data.buy) }}
                      </span></span></h4>
                </div>
                <div @click="$router.push('/admin-credit')" v-if="supplier_data.tbs > 0" class="info-row">
                  <h4><span class="text-success label">TBS Credit Amount <span
            ...
            <b-modal size="lg" id="modal-prevent-closing-price" centered ref="my-modal-price"
              title="Invoice Order Details" ok-title="Submit" cancel-variant="outline-secondary" @show="resetModalPrice"
              ...
              <form ref="form" @submit.stop.prevent="handleSubmitPrice">
                  ...
                  <div v-for="(product, index) in sale_item_carts" :key="product.id" class="item-row">
                    <b-row class="align-items-center no-gutters">
                      ...
                      <b-col cols="2" class="text-right">
                        <h6 class="display-new-4 mb-0">
                          {{ formatPrice(product.purchas_price) }}
                        </h6>
                      </b-col>
                ...
                <div class="payment-box">
                  <div class="payment-row display-new-4">
                    <span>Gross Amount </span>
                    <span> {{ formatPrice(sub_sum_total) }}</span>
                  </div>

                  <div class="payment-row display-new-4">
                    <span> Trans. & Labour</span>
                    <span v-if="transport > 0"> {{ formatPrice(transport) }}</span>
                    <span v-else> 0.00</span>
                  </div>

                  <div class="payment-row display-new-4">
                    <span> Loan Charge </span>
                    <span v-if="labour > 0"> {{ formatPrice(labour) }}</span>
                    <span v-else> 0.00</span>
                  </div>
            ...
            <div class="d-flex justify-content-between mb-3">
              ...
              <span class="font-weight-bold">Rs {{ formatPrice(sub_sum_total) }}</span>
            </div>
            ...
            <div v-if="supplier_data.id == 1" class="input-row">
              <div class="d-flex align-items-center">
                ...
                <span class="hold">Trans. & Labour</span>
              </div>
              <b-form-input type="number" min="0" step="0.01" v-model.number="transport" class="input-box"
            ...
            <div v-else class="input-row">
              ...
              <b-form-input ref="transportInput" v-model="transport" type="number" min="0" step="0.01" class="input-box"
                @change="getTotalSalePrice" />
            </div>
            ...
            <div class="input-row">
              ...
              <b-form-input v-model="labour" type="number" min="0" step="0.01" class="input-box"
                @change="getTotalSalePrice" />
            </div>
            ...
            <div class="input-row">
              ...
              <b-form-input disabled type="number" min="0" step="0.01" v-model.number="cash" class="input-box"
                @change="getTotalSalePrice" />
            </div>
            ...
            <div class="input-row">
              <div class="d-flex align-items-center">
                ...
                <span>CHQ Payment</span>
              </div>
              <b-form-input ref="chqInput" v-model="cheque" type="number" min="0" step="0.01" class="input-box"
                @change="getTotalSalePrice" />
            </div>
            ...
            <div class="input-row">
              <div class="d-flex align-items-center text-danger">
                ...
                <span class="ppcredit">Credit</span>
              </div>
              <b-form-input v-model="credit" type="number" min="0" step="0.01" class="input-box"
                @change="getTotalSalePrice" />
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
              ...
              <span v-if="balance != 0" class="text-danger font-weight-bold">
            ...
            <!-- Button -->
            <b-button block class="payment-btn" :disabled="cash < 0 || sub_sum_total == 0 || isLoading || isQtyActive"
              @click="submitNewData">
            ...
            <div class="tip-box mt-3">
              💡 Tip: You can add custom items if the item is not in the list.
            </div>
        ...
        <b-card no-body class="invoice-preview-card">
          <b-row>
            <b-col cols="12">
              ...
              <div v-if="sale_item_carts.length <= 16" style="height: 740px;" class="mt-1">
                <div v-for="(product, index) in sale_item_carts" :key="product.id" class="item-row">
                  <b-row class="align-items-center no-gutters">
                ...
                <b-col cols="6" class="pr-5">
                  <div class="payment-box">
                    ...
                    <div class="payment-row display-new-70">
                      ...
                      <span v-if="cheque > 0"> {{ formatPrice(cheque) }}</span>
                      <span v-else> -</span>
                    </div>

                    <div class="payment-row display-new-70">
                      <span> Card Payment Rs.</span>
                      <span v-if="hold > 0"> {{ formatPrice(hold) }}</span>
                      <span v-else> -</span>
                    </div>

                    <div class="payment-row display-new-73">
                      <span> Credit Rs.</span>
                      <span v-if="credit > 0"> {{ formatPrice(credit) }}</span>
                      <span v-else> -</span>
                    </div>
                    <!-- <div v-if="hold > 0" class="payment-row display-new-70">
                      <span>Company Hold Rs.</span>
                      <span> {{ formatPrice(hold) }}</span>
                    </div>
...
export default {
  ...
  computed: {
    ...
    processedSaleCarts() {
      return this.sale_item_carts.map(product => {
  ...
  methods: {
    ...
    async groupData() {
      try {
        const response = await product.shopItemData()
    ...
    async submitNewData() {
      const driverLoan = Number(this.driver_data?.loan || 0)
      const supplierLoan = Number(this.supplier_data?.loan || 0)
      const labour = Number(this.labour || 0)

      if (labour <= driverLoan || labour <= supplierLoan) {
        ...
        try {
          ...
          this.isLoading = true
          await item.submitSaleCart(this.payload)
          this.printInvoice()
          this.transport = 0
          this.labour = 0
          this.cash = 0
        ...
        this.showErrorMessage('Loan Charge must be less than or equal to Outstanding Loan')
      }
    ...
    async updateSaleQuentityCart(id, quentity) {
      ...
      try {
        ...
        await item.updateSaleQuentityCart(id, payload)
    ...
    async deleteSaleCartLastItem() {
      ...
      await item.deleteSaleCartLastItem(this.$route.params.id)
      await this.getAdminSaleCarts()
      if (this.$refs.nameInput) {
        if (this.$refs.nameInput) {
          this.$refs.nameInput.focus()
    ...
    async addtocart(project) {
      ...
      try {
        ...
        this.isLoading = true
        const response = await item.addProductToSaleCart(this.payload)
    ...
    async deleteOrder() {
      ...
      try {
        ...
        this.$swal({
          ...
          if (result.value) {
            await item.deletePurchasOrder(this.order_data.id)
    ...
    async getAdminSaleCarts() {
      try {
        const response = await item.getAdminPOSSaleCarts(this.$route.params.id)
    ...
    async getOrderDetails() {
      try {
        const response = await order.getOrderDetails(this.$route.params.id)
    ...
    async getTotalPrice() {
      this.sub_total = this.sale_carts
        .map(cart => parseFloat(cart.quentity * cart.purchas_price))
        .reduce((total, quentity) => total + quentity, 0);

      this.sale_total = this.sub_total;

      if (this.transport < 0 || this.transport === null || this.transport === undefined || this.transport === '' || isNaN(this.transport)) {
        this.transport = 0
      }

      if (this.labour < 0 || this.labour === null || this.labour === undefined || this.labour === '' || isNaN(this.labour)) {
        this.labour = 0
      }
      ...
      if (this.hold < 0 || this.hold === null || this.hold === undefined || this.hold === '' || isNaN(this.cash)) {
        this.hold = 0
      }

      if (this.cheque < 0 || this.cheque === null || this.cheque === undefined || this.cheque === '' || isNaN(this.cheque)) {
        this.cheque = 0
      }
      ...
      this.sum_total = parseFloat(this.sale_total) - (parseFloat(this.transport) + parseFloat(this.labour));
      this.cash = parseFloat(this.sum_total) - (parseFloat(this.hold) + parseFloat(this.cheque) + parseFloat(this.credit));
      this.balance = parseFloat(this.cash) + parseFloat(this.cheque) + parseFloat(this.credit) + parseFloat(this.hold) - parseFloat(this.sum_total);
