                ...
                <b-col sm="12" md="12" lg="6">
                  <b-form-group label="User Name" label-for="name">
                    <validation-provider #default="{ errors }" name="User Name" rules="required">
                      <b-form-input id="name" v-model="form.supplier_name" :state="errors.length > 0 ? false : null"
...
export default {
  ...
  setup() {
    ...
    const statusOptions = [
      { label: 'Active Supplier', value: 1 },
      { label: 'InActive Supplier', value: 2 },
    ]
  ...
  methods: {
    ...
    async formSubmitted() {
      ...
      try {
        ...
        this.showSuccessMessage('Update Supplier Successfully')
        this.$router.push('/suppliers')
        this.isLoading = false
      } catch (error) {
        this.errorUserName = error.response.data.data.supplier_name ? error.response.data.data.supplier_name[0] : ''
    ...
    async userDetails() {
      try {
        const response = await admin.getSupplierDetails(this.$route.params.id)
        ...
        this.showErrorMessage('Supplier not found.')
      }
    },
    ...
    async uploadImages() {
      try {
        ...
        this.isLoading = true;
        await admin.uploadSupplierImage(formData, {
          headers: {
