    ...
    <b-col cols="12">
      <b-overlay :show="tableLoading" rounded="sm">
        ...
        <template #cell(users_user_name)="data">
            <div> {{ data.item.users_user_name }}</div>
          </template>
          ...
          <template #cell(credit_type)="data">
            <b-badge v-if="data.item.credit_type == '1' " class="ml-1" style="padding: 8px" variant="light-primary">
                Sell Order
            </b-badge>
            <b-badge v-if="data.item.credit_type == '4' " class="ml-1" style="padding: 8px" variant="light-success">
                Plastic Sell Order
            </b-badge>
        ...
        <template  #cell(actions)="data">
                ...
                <div  class="text-nowrap">
                  <b-button v-if="data.item.credit_type == '1' "
                ...
                <feather-icon
                  ...
                  icon="EyeIcon"
                  size="16"
                  @click="$router.push({ name: 'view-payment-payment-sale-order', params: { id: data.item.order_id }})"
                />
                </b-button>
                  <b-button v-if="data.item.credit_type == '4' &&  data.item.return_items_count === null"
                                  v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                                  variant="outline-primary"
                                  class="btn-icon mr-50"
                                >
                <feather-icon
                  ...
                  icon="EyeIcon"
                  size="16"
                  @click="$router.push({ name: 'admin-return-orders'})"
                />
                </b-button>
                <b-button v-if="data.item.credit_type == '4' &&  data.item.return_items_count !== null"
                                  v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                ...
                <feather-icon
                  ...
                  icon="EyeIcon"
                  size="16"
                  @click="$router.push({ name: 'view-payment-hardware-return-order-list', params: { id: data.item.order_id }})"
                />
                </b-button>
  ...
  methods: {
    async getUsers() {
      try {
        ...
        const Response = await payment.getPastIncomeCredits(
