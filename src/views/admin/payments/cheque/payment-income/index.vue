<template>
    ...
    <b-row class="m-2">
      <b-col cols="12">
        <b-overlay :show="tableLoading" rounded="sm">
          ...
          >
            ...
            <template #cell(cheque_type)="data">
                <b-badge v-if="data.item.cheque_type == '1' " class="ml-1" style="padding: 8px" variant="light-primary">
                    Sell Order
                </b-badge>
                <b-badge v-if="data.item.cheque_type == '4' " class="ml-1" style="padding: 8px" variant="light-success">
                    Plastic Sell Order
                </b-badge>
      ...
      <template  #cell(actions)="data">
        ...
        <div v-if="data.item.active_status == 1" class="text-nowrap">
          <b-button
            variant="success"
            @click="$router.push({ name: 'edit-cheque-today-income-details', params: { id: data.item.id }})"
          >
          <span class="align-middle">Edit</span>
          </b-button>
          <b-button
           v-if="data.item.due_date <= formattedDate"
            class="ml-1" variant="primary"
             @click="pay(data.item.id, data.item.cheque_type)"
          >
          <span class="px-2 align-middle">Pay</span>
          </b-button>
        ...
        <div v-if="data.item.active_status == 2">
                    {{ data.item.paid_date }}
                </div>
        </template>
  ...
  export default {
    ...
    setup() {
      const statusOptions = [
        { label: 'All', value: 0 },
        { label: 'Sell Order', value: 1 },
        { label: 'Plastic Sell Order', value: 4 },
      ]
    ...
    methods: {
      ...
      async getUsers() {
        try {
          ...
          const Response = await payment.getPaymentIncomeCheques(
          ...
          try {
            ...
            this.$swal({
              ...
              if (result.value) {
               ...
               await payment.payIncomeCheque(id, this.payload)
                this.showSuccessMessage('Cheque Amount has been paied')
                this.refreshTable()
                this.getUsers()
  ...
  try {
    ...
    this.$swal({
      ...
      if (result.value) {
       await payment.cancelExpenditureCheque(id)
        this.showSuccessMessage('Cheque Amount has been canceld')
        this.refreshTable();
      } else if (result.dismiss == 'cancel') {
        this.formLoading = false
        this.showSuccessMessage('Cancelled')
