  ...
  <div class="fixed-page">
    <section class="invoice-preview-wrapper">
      <b-card no-body class="invoice-actions p-1 bg-primary rounded p-1">
        <b-row>
          <b-col cols="1">
            <feather-icon @click="$router.push('/order_list')" icon="ChevronLeftIcon" class="text-white" size="54" />
          </b-col>
          <b-col cols="10">
            <h3 class="text-white d-flex justify-content-center">
              <span class="mt-1 text-white">Today Sale Order ( {{ orders.purchas_orders_bill_number }} ) </span>
            </h3>
          </b-col>
        </b-row>
      </b-card>
      <b-row class="invoice-actions">
        <b-col lg="6" md="6" sm="12" class="mb-2">
          <div class="ml-2">
            ...
            <b-card-text>
              {{ orders.purchas_orders_bill_number }}
            </b-card-text>
          </div>
        ...
        <b-col lg="6" md="6" sm="12" class="mb-2">
          ...
          <div class="ml-2">
            ...
            <b-card-text>
              <div>
                Rs: {{ (orders.purchas_orders_total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
              </div>
            </b-card-text>
          </div>
          <div class="ml-2">
            ...
            <b-card-text>
              {{ orders.purchas_orders_order_date }} | {{ orders.purchas_orders_order_time }}
            </b-card-text>
          </div>
          <div class="ml-2 mb-1">
            <h4 class="text-capitalize mt-2">
          ...
          <!-- <span class="ml-1">
              <b-button v-if="orders.purchas_orders_active_status == 0" :class="{ 'btn-loading': isLoading }"
                :disabled="isLoading" type="submit" variant="primary" @click="addStore(orders.purchas_orders_id)">
          ...
          <span class="ml-1">
            <b-button v-if="orders.purchas_orders_active_status == 0" :class="{ 'btn-loading': isLoading }"
              :disabled="isLoading" type="submit" variant="danger" @click="deleteOrder(orders.purchas_orders_id)">
              <span v-if="isLoading">Loading...</span>
      ...
      <b-card style="height: 805px;" class="invoice-actions">
        ...
        <div>
          <h4 class="d-flex align-items-center justify-content-start mb-1">
            Order Items List : ( Rs: {{ (orders.purchas_orders_total).toString(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }}
            )
          </h4>
        </div>
        <table style="height: 605px;" class="table table-hover shopping-cart-wrap table-responsive">
          ...
          <tbody>
            <tr v-for="product in item_orders" :key="product.purchas_orders_id">
              <td>
                <figure class="media">
                  ...
                  <b-row>
                    ...
                    <b-col cols="5">
                      ...
                      <vue-barcode class="mt-1 d-flex justify-content-end barcode"
                        :value="orders.purchas_orders_bill_number" :format="format" :height="55"
                        :display-value="false" />
                      <div>
                  ...
                  <b-row>
                    ...
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8">
                        Invoice No <span class="ml-3">: {{ orders.purchas_orders_bill_number }} </span>
                      </h6>
                      <h6 class="font-weight-bolder display-new-8">
                    ...
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8 d-flex justify-content-end">
                        Invoice Date <span class="ml-3"> : {{ formatDate(orders.purchas_orders_order_date) }} </span>
                      </h6>
                      <h6 class="font-weight-bolder display-new-8 d-flex justify-content-end">
                        {{ formatTime(orders.purchas_orders_order_time) }}
                      </h6>
                    </b-col>
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8">
                    ...
                    <b-col cols="6">
                      <h6 class="font-weight-bolder display-new-8 d-flex justify-content-end">
                        Lorry Number <span class="ml-1"> <span class="ml-50">: {{ orders.purchas_orders_lorry_number
                            }} </span></span>
                      </h6>
                    </b-col>
                  ...
                  <b-col cols="6" class="pr-5">
                    <div class="payment-box">
                      <div class="payment-row display-new-70">
                        ...
                        <span> {{ formatPrice(orders.purchas_orders_cash) }}</span>
                      </div>

                      <div class="payment-row display-new-70">
                        <span> CHQ Payment Rs.</span>
                        <span v-if="orders.purchas_orders_cheque > 0"> {{ formatPrice(orders.purchas_orders_cheque)
                          }}</span>
                        <span v-else> -</span>
                      </div>
                      <div class="payment-row display-new-70">
                        <span> Card Payment Rs.</span>
                        <span v-if="orders.purchas_orders_hold > 0"> {{ formatPrice(orders.purchas_orders_hold)
                          }}</span>
                        <span v-else> -</span>
                      </div>
                      ...
                      <div class="payment-row display-new-73">
                        <span> Credit Rs.</span>
                        <span v-if="orders.purchas_orders_credit > 0"> {{ formatPrice(orders.purchas_orders_credit)
                          }}</span>
                        <span v-else> -</span>
                      </div>
                      <!-- <div v-if="orders.purchas_orders_hold > 0" class="payment-row display-new-70">
                      <span>Company Hold Rs.</span>
                      <span> {{ formatPrice(orders.purchas_orders_hold) }}</span>
                    </div>
                  ...
                  <b-col cols="6" class="pl-5">
                    <div class="payment-box">
                      <div class="payment-row display-new-70">
                        ...
                        <span> {{ formatPrice(orders.purchas_orders_subtotal) }}</span>
                      </div>

                      <div class="payment-row display-new-70">
                        <span> Trans. & Labour </span>
                        <span v-if="orders.purchas_orders_transpoart > 0"> {{
                          formatPrice(orders.purchas_orders_transpoart) }}</span>
                        <span v-else> 0.00</span>
                      </div>

                      <div class="payment-row display-new-70">
                        <span> Loan Charge </span>
                        <span v-if="orders.purchas_orders_labour_charge > 0"> {{
                          formatPrice(orders.purchas_orders_labour_charge) }}</span>
                        <span v-else> 0.00</span>
                      </div>

                      <div class="payment-row display-new-73">
                        <span>Net Total Rs.</span>
                        <span> {{ formatPrice(orders.purchas_orders_total) }}</span>
                      </div>
                    </div>
                  </b-col>
                </b-row>
