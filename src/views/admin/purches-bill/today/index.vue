<template>
  <div>
    <b-card>
      <h2> Today Orders : {{ pagination.totalRows }} </h2>
              ...
              <template #cell(total)="data">
                <div class="d-flex justify-content-end">
                  {{ data.item.total }}
                      ...
                      <template #cell(labour_charge)="data">
                <div class="d-flex justify-content-end text-primary">
                  {{ data.item.labour_charge }}
                </div>
              </template>
                ...
                <div class="text-nowrap">
                  <b-button v-if="data.item.active_status != 3" v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                    ...
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16"
                      @click="$router.push({ name: 'view-payment-today-sale-order', params: { id: data.item.id } })" />
                  </b-button>
                  <b-button v-if="userData.admin_role == 'TBSAdmin' && (data.item.active_status == 1 || data.item.active_status == 2)"
                    @click="deleteOrder(data.item.id)" v-ripple.400="'rgba(255, 255, 255, 0.15)'"
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getTodaySalesOrder(
          this.filterQuery, this.currentPage, this.pagination.perPage, this.sortBy, this.sortDesc);
        const dataArray = Response.data.data.map((x) => ({
          id: x.purchas_orders_id,
          order_date: x.purchas_orders_order_date,
          cashier_id: x.purchas_orders_cashier_id,
          users_user_name: x.users_user_name,
          suppliers_supplier_name: x.suppliers_supplier_name,
          bill_number: x.purchas_orders_bill_number,
          transpoart: x.purchas_orders_transpoart,
          labour_charge: x.purchas_orders_labour_charge,
          order_time: new Date('1970-01-01T' + x.purchas_orders_order_time)
            .toLocaleTimeString('en-US', {
              hour: '2-digit',
              minute: '2-digit',
              hour12: true
            }),
          subtotal: 'Rs: ' + Number(x.purchas_orders_subtotal).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          transpoart: 'Rs: ' + Number(x.purchas_orders_transpoart).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          labour_charge: 'Rs: ' + Number(x.purchas_orders_labour_charge).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          credit: 'Rs: ' + Number(x.purchas_orders_credit).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),

          total: 'Rs: ' + Number(x.purchas_orders_total).toLocaleString('en-LK', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          }),
          active_status: x.purchas_orders_active_status
        }));
