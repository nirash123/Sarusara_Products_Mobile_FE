<template>
  <b-card>
    <!-- media -->
    <b-media no-body>
      <b-button @click="$router.push('/shop-items')" v-ripple.400="'rgba(40, 199, 111, 0.15)'" variant='light-dark'
        ...
        <b-col cols="12">
              ...
              <b-row>
                ...
                <b-col sm="12" md="12" lg="6">
                  <b-form-group label="Category" label-for="group-input" invalid-feedback="Category is required">
                    <v-select id="group-input" v-model="form.category"
                      :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" label="category_name" :options="groups"
          ...
          <b-col  cols="12" class="mt-2 d-flex justify-content-end gap-2">
            <span class="mr-2">
             ...
             <b-button class="modern-reset-btn"
  ...
  methods: {
    ...
    async formSubmitted() {
      ...
      try {
        ...
        this.isLoading = true
        const response = await product.shopProductUpdate(this.$route.params.id, this.payload)
        ...
        error.response.data.data.category ? this.showErrorMessage('The category is required.') : ''
        this.isLoading = false
      }
    ...
    async userDetails() {
      try {
        const response = await product.shopProductDetails(this.$route.params.id)
