  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await product.getAdminShopProducts(
    ...
    async deleteProduct(id) {
      ...
      try {
        ...
        this.$swal({
          ...
          if (result.value) {
            await product.shopProductDelete(id)
    ...
    async subItemData() {
      if (this.category && this.category.id) {
        try {
          this.item_name = this.category.category_name
        } catch (error) {
    ...
    async companyPrice(item_code, item_name, category, quentity,company_price, sale_price) {
      try {
        this.payload = {
          item_code: item_code,
          item_name: item_name,
        ...
        this.isLoading = true
        const response = await product.shopProductCreate(this.payload)
