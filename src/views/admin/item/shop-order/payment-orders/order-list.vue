<template>
  <div>
    <b-card no-body class="p-1 bg-primary rounded p-1">
            <b-row>
              <b-col cols="1">
               <feather-icon @click="$router.push('/shop-ordres?tab=2')" icon="ChevronLeftIcon" class="text-white" size="54"/>
              </b-col>
              <b-col cols="10">
                <h3 class="text-white d-flex justify-content-center">
              <span class="mt-1 text-white">Payment Pending Order (  H_ORD  {{ this.$route.params.id }} ) </span></h3>
              </b-col>
            </b-row>
          </b-card>
  <b-card>
    ...
    checkFormValidityPrice() {
      ...
      if(this.formattedPrice >= (parseFloat(new_cash) + parseFloat(new_card) + parseFloat(new_cheque) ) ){
        ...
        try {
                ...
                this.isLoading = true
                const response = await item.submitHardwareNewCredit(this.$route.params.id, this.payload)
                this.showSuccessMessage('Add Payment Successfully')
                await this.hardwareOrderDetails()
                this.$refs.table.refresh();
