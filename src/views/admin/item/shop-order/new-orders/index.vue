<template>
  ...
  <b-row class="my-2">
    ...
    <feather-icon
      ...
      icon="EyeIcon"
      size="16"
      @click="$router.push({ name: 'view-hardware-order-list', params: { id: data.item.id }})"
    />
    </b-button>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getHardwareNewOrder(
    ...
    checkFormValidityPrice() {
      ...
      if(parseInt(this.bank_amount) >= parseInt(cash)){
      ...
      if((parseFloat(subtotal) - parseFloat(discount)) == (parseFloat(cash) + parseFloat(card) + parseFloat(cheque) + parseFloat(credit)) ){
        ...
        try {
                ...
                this.isLoading = true
                const response = await item.submitHardwareNewCart(this.payload)
    ...
    async deleteOrder(id) {
      ...
      try {
        ...
        this.$swal({
          ...
          if (result.value) {
          await item.deleteHardwareNewCart(id)
