<template>
  ...
  <feather-icon
      ...
      <div class="profile-title ml-3">
        <h1 class="font-large-2 text-white">
          H_ORD {{ orders.id }} | {{ orders.shops_shop_name }}
        </h1>
        ...
        <b-col cols="12" md="4">
            <b-row>
                      ...
                      <b-row v-if="carts.length < 20" @click="pos()">
                          <b-col cols="12">
                                <b-row>
                                  <b-col sm="12" md="6" lg="6" v-for="project in getData" :key="project.id">
           ...
           <b-card  no-body class="px-1">
               <div  class="card">
                    ...
                    <table style="height: 750px;" class="table table-hover shopping-cart-wrap table-responsive">
                       <thead>
                          <tr>
                             ...
                             <th scope="col" width="300">Comapny Price</th>
                             <th scope="col" width="300">Sale Price</th>
                             <th scope="col" width="150">Qty</th>
                             <th scope="col" width="1000">Expired Date</th>
                          ...
                          <tr v-for="product in carts" :key="product.id" >
                                ...
                                <figure class="media">
                                   <figcaption class="media-body">
                                      <b-form-input
                                           ...
                                           @change="updateHardwareCompanyPrice(product.id, product.company_price)"
                                       />
                                   </figcaption>
                                </figure>
                             </td>
                                ...
                                <figure class="media">
                                   <figcaption class="media-body">
                                      <b-form-input
                                           ...
                                           @change="updateHardwareSalePrice(product.id, product.sale_price, product.company_price)"
                                       />
                                   </figcaption>
                                </figure>
                             </td>
                                ...
                                <figure class="media">
                                   <figcaption class="media-body">
                                      <b-form-input
                                           ...
                                           @change="updateHardwareQuentity(product.id, product.quentity)"
                                       />
                                   </figcaption>
                                </figure>
                             </td>
                             <td>
                              <div v-if="product.expired_date_status == 1">
                                <b-form-datepicker
                                      id="example-datepicker"
                                      v-model="product.expired_date"
                                      placeholder="YYYY-MM-DD"
                                      class="mb-1"
                                      @input="updateExpiredDate(product.id, product.expired_date)"
                                    />
                                  </div>
                             </td>
                             <td>
                             ...
                             <td class="text-right"> 
                               <b-button
                                       ...
                                       @click="deleteHardwareCart(product.id)"
                                       >
                                       <feather-icon
                                           icon="TrashIcon"
                                       />
                       ...
                       <div class="box mt-1 px-3">
                        ...
                        <b-row >
                              ...
                              <b-col cols="6">
                                <dd  class="text-right"><h4>Rs: {{((cart_sum - orders.subtotal ).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}}</h4></dd>
                              </b-col>
                              <b-col cols="6">
                                <dt class="b my-25"><h2>Total: </h2></dt>
                              </b-col>
                              <b-col cols="6">
                                <dd  class="text-right h4 b my-25"><h2> Rs: {{(orders.subtotal).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}}</h2></dd> 
                               </b-col>
                          </b-row>
                          </div>
                 ...
                 <div class="text-right">
                  <b-button
                   v-if="(cart_sum - orders.subtotal) <= 100 && (cart_sum - orders.subtotal) >= -100"
  ...
  export default {
   ...
   methods: {
    ...
    async getAdminHardwareCarts(){
         try {
         const response = await item.getAdminHardwareCarts(JSON.parse(localStorage.getItem('userData')).id)
     ...
     async getAdminHardwareOrderDetails(){
         try {
         const response = await item.getAdminHardwareOrderDetails(this.$route.params.id)
    ...
    this.$refs.nameInput.focus();
     ...
     async addQRCode(item_code){
      ...
      try {
         ...
         this.isLoading = true
         const response = await item.addQRToHardwareCart(this.payload)
         this.getAdminHardwareCarts()
         this.getItems()
         this.item_code = ''
         this.$refs.nameInput.focus();
      ...
      const formattedMonth = year + '-' + month;
       ...
       try {
          ...
          this.payload = {
           ...
           this.isLoading = true
           const response = await item.submitHardwareCart(this.payload)
           this.showSuccessMessage('Submit Product Successfully')
           this.$router.push('/shop-ordres?tab=2')
          this.getAdminHardwareCarts()
          this. getItems()
          this.shop_id = null
   ...
   async updateHardwareCompanyPrice(id,company_price){
       try {
           this.payload = {
             id: id,
           ...
           this.isLoading = true
           await item.updateHardwareCompanyPrice(id,this.payload)
           this.getAdminHardwareCarts()
           this.$refs.nameInput.focus();
           this.isLoading = false
         } catch (error) {
     ...
     async updateHardwareSalePrice(id,sale_price, company){
      if(sale_price > company){
       try {
           this.payload = {
             id: id,
           ...
           this.isLoading = true
           await item.updateHardwareSalePrice(id,this.payload)
           this.getAdminHardwareCarts()
           this.$refs.nameInput.focus();
           this.isLoading = false
         } catch (error) {
     ...
     async updateHardwareQuentity(id,quentity){
       try {
           this.payload = {
             id: id,
             quentity: quentity,
           ...
           this.isLoading = true
           await item.updateHardwareQuentity(id,this.payload)
           this.getAdminHardwareCarts()
           this.$refs.nameInput.focus();
           this.isLoading = false
         } catch (error) {
     ...
     async updateExpiredDate(id,expired_date){
       try {
           this.payload = {
             id: id,
           ...
           this.isLoading = true
           await item.updateExpiredDate(id,this.payload)
           this.getAdminHardwareCarts()
           this.$refs.nameInput.focus();
           this.isLoading = false
         } catch (error) {
     ...
     async deleteHardwareCart(id){
  
          await item.deleteHardwareCart(id)
           this.getAdminHardwareCarts()
           this.$refs.nameInput.focus();
           this.isLoading = false
     },
     ...
     async addtocart(id){
        ...
        try {
           ...
           this.isLoading = true
           const response = await item.addProductToHardwareCart(this.payload)
           this.getAdminHardwareCarts()
           this.$refs.nameInput.focus();
           this.isLoading = false
         } catch (error) {
      ...
      try {
        ...
        this.getdataLength = dataArray.length
   ...
   mounted() {
     if(JSON.parse(localStorage.getItem('userData')).admin_role == 'TBSAdmin' || JSON.parse(localStorage.getItem('userData')).admin_role == 'TBSAccountant') {
       this.getAdminHardwareOrderDetails()
       this.getItems()
       this.getAdminHardwareCarts()
       this.getAdmin()
       this.pos() 
