<template>
  ...
  <b-row class="my-2">
    ...
    <feather-icon
      ...
      icon="EyeIcon"
      size="16"
      @click="$router.push({ name: 'view-completed-hardware-order-list', params: { id: data.item.id }})"
    />
    </b-button>
    <b-button
                  v-ripple.400="'rgba(255, 255, 255, 0.15)'"
  ...
  <feather-icon
    ...
    icon="EditIcon"
    size="16"
    @click="$router.push({ name: 'view-hardware-completed-invoice', params: { id: data.item.id }})"
  />
  </b-button>
    </div>
    </template>
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        const Response = await order.getHardwareCompletedOrder(
